...
openModalBtn?.addEentListener('click', async function () {
    try {
        ...
        const ratingAndTime = `${data.rating.toFixed(1)} ${generateRatingStars(data.rating)} | ${data.time} min`;

        const ingredientsResponse = await fetch(`https://tasty-treats-backend.p.goit.global/api/ingredients`);
        const ingredientsData = await ingredientsResponse.json();

        const ingredientItems = data.ingredients.map(ingredientData => {
            const ingredientInfo = ingredientsData.find(item => item._id === ingredientData.id);
            const ingredientName = ingredientInfo ? ingredientInfo.name : 'Unknown Ingredient';
            return `
        ...
        function extractVideoId(videoUrl) {
            const match = videoUrl.match(/(?:\/embed\/|v=|\/v\/|youtu\.be\/|\/v=|^v=)([-_a-zA-Z0-9]+)/i);
